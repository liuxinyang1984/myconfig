    ...
    if type "xrandr"; then
      ...
      for m in $(xrandr --query | grep " connected" | cut -d" " -f1); do
        if [[ "$style" == "hack" || "$style" == "cuts" ]]; then
            echo "MONITOR=$m polybar -q top -c "$dir/$style/config.ini" &"
            MONITOR=$m polybar -q top -c "$dir/$style/config.ini" &
            MONITOR=$m polybar -q bottom -c "$dir/$style/config.ini" &
        elif [[ "$style" == "pwidgets" ]]; then
            echo "bash MONITOR=$m polybar -q top -c "$dir/$style/config.ini" &"
            bash "$dir"/pwidgets/launch.sh --main
        else
            echo "MONITOR=$m polybar -q main -c "$dir/$style/config.ini" &"
